...
counties_monthly <- county_month_aggregated %>% 
  distinct(county_geoid_coalesced) %>% 
...
# total number of grantees, by grantee_geographic_level

county_month_selected_grantees %>% 
  group_by(geographic_level, threshold_passing_with_complete_geography) %>%
  summarize(number_of_grantees = n ()) %>% 
  mutate(percent_included = number_of_grantees / sum(number_of_grantees)) %>% 
  ungroup() %>% 
  filter(threshold_passing_with_complete_geography) %>% 
  ...
  datatable() %>% 
  formatPercentage("percent_included", 
                   digits = 3)
