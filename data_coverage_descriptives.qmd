...
county_month_pre_agg_joined %>%
  ...
  left_join(counties, by = join_by("grantee_state" == "STATE_NAME")) %>%
  distinct() %>%
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# total number of passing grantees, by grantee_geographic_level monthly
county_month_selected_grantees %>%
...
ungroup() %>% 
select(geographic_level, passing_total, pct_of_total_passing) %>% 
rename(n_passing = passing_total,
pct_grantees_passing = pct_of_total_passing
) %>% 
  datatable() %>% 
  formatPercentage("pct_grantees_passing", 
                   digits = 3)
```
...
county_month_selected_grantees %>%
    group_by(geographic_level, grantee_state) %>%
    mutate(total = n()) %>%
    filter(threshold_passing_with_complete_geography == TRUE) %>% 
    group_by(threshold_passing_with_complete_geography, geographic_level, grantee_state) %>%
    summarise(
        passing_total = n(),
        total = first(total),
        pct_of_total_passing = passing_total/total) %>% 
ungroup() %>% 
select(geographic_level,grantee_state, passing_total, pct_of_total_passing) %>% 
rename(n_passing = passing_total,
pct_grantees_passing = pct_of_total_passing
) %>% 
  datatable() %>% 
